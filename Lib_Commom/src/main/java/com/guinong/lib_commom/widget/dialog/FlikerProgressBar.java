...
public class FlikerProgressBar extends View implements Runnable{
    ...
    private int DEFAULT_HEIGHT_DP = 10;
    ...
    private void initAttrs(AttributeSet attrs) {
        ...
        try {
            textSize = (int) ta.getDimension(R.styleable.FlikerProgressBar_mtextSize, 12);
    ...
    private void init() {
        ...
        int bitWidth = flikerBitmap.getWidth();
        int bitHeight = flikerBitmap.getHeight();

        float scaleHeight = getMeasuredHeight()/(bitHeight*1.0f);

        // 取得想要缩放的matrix參數
        Matrix matrix = new Matrix();
        matrix.postScale(1, scaleHeight);
        // 得到新的圖片
         flikerBitmap = Bitmap.createBitmap(flikerBitmap, 0, 0, bitWidth, bitHeight, matrix,true);
        initPgBimap();
    }
    ...
    private void drawProgress(Canvas canvas) {
        ...
        if(!isStop){
            ...
            pgCanvas.drawBitmap(flikerBitmap, flickerLeft, (getMeasuredHeight()-flikerBitmap.getHeight())/2, pgPaint);
