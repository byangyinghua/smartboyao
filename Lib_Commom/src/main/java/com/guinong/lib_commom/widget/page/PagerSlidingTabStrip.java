package com.guinong.lib_commom.widget.page;
...
public class PagerSlidingTabStrip extends HorizontalScrollView {
    ...
    private int tabSeletTextSize = 15;
    ...
    private Map<Integer, View> viewdata = new HashMap<>();
    ...
    public PagerSlidingTabStrip(Context context, AttributeSet attrs, int defStyle) {
        ...
        tabSeletTextSize = a.getDimensionPixelSize(R.styleable.PagerSlidingTabStrip_pstsTextSelectedSize, 20);
        ...
        mTextPaddingTop = a.getDimensionPixelSize(R.styleable.PagerSlidingTabStrip_pstsTextPaddingTop, mTextPaddingTop);
        mTextPaddingBottom = a.getDimensionPixelSize(R.styleable.PagerSlidingTabStrip_pstsTextPaddingBottom, mTextPaddingBottom);
        a.recycle();
    ...
    public void setTabLayoutParams(int width) {
        if (shouldExpand) {
            expandedTabLayoutParams = new LinearLayout.LayoutParams(width, LayoutParams.MATCH_PARENT);
        } else {
...
//                        mPaintTabText.setTextSize(tabTextSize);
                        mPaintTabText.setTextSize(AutoUtils.getPercentWidthSize(tabTextSize));
                        mTabsTextWidth += (int) mPaintTabText.measureText(s, 0, s.length());
                    }
                    if (mTabsTextWidth > 0) {
                        tabPadding = (getMeasuredWidth() - mTabsTextWidth) / tabCount / 2;
                        Log.e("yangmbin", "tabPadding:" + tabPadding);
                    }
                }

                updateTabStyles();
                Log.e("yangmbin", "update.....");
            }
        });
    }
    ...
    private void addTab(final int position, View tab, View tab2) {
        ...
        final TitleView titleView = new TitleView(getContext());
        ...
        viewdata.put(position, titleView);//统计控件到map中
     ...
     * @param postion
     */
    private void setTextBackColor(int postion) {
        if (viewdata.get(postion) != null) {
...
//                    tab.setTextSize(TypedValue.COMPLEX_UNIT_PX, tabTextSize);
                    tab.setTextSize(TypedValue.COMPLEX_UNIT_PX, AutoUtils.getPercentWidthSize(tabTextSize));
                    tab.setTypeface(tabTypeface, tabTypefaceStyle);
                    tab.setEllipsize(TextUtils.TruncateAt.END);
                    if (tabTypefaceStyle == Typeface.BOLD) {
                        TextPaint tp = tab.getPaint();
                        tp.setFakeBoldText(true);
                    }
                    tab.setPadding(tabPadding, mTextPaddingTop, tabPadding, mTextPaddingBottom);
                    if (j == 0) {
                        tab.setTextColor(tabTextColor);

                    } else {
                        tab.setTextColor(selectedTabTextColor);
                        tab.setTextSize(tabSeletTextSize);
                    }
...
//            canvas.drawRect(lineLeft+tabPadding, height - indicatorHeight, lineRight-tabPadding, height, rectPaint);
            if (mIndicatorWidth > 0) {
                int len = Math.abs((int) (lineRight - tabPadding - lineLeft - tabPadding));
                if (mIndicatorWidth < len)
                    canvas.drawRect(lineLeft + tabPadding + mIndicatorWidth / 2, height - indicatorHeight, lineRight - tabPadding - mIndicatorWidth / 2, height, rectPaint);
            } else
                canvas.drawRect(lineLeft + tabPadding, height - indicatorHeight, lineRight - tabPadding, height, rectPaint);
        }
    ...
    protected void animateFadeScale(View left, View right, float positionOffset, int position) {
        if (mState != State.IDLE) {
            if (left != null) {
                ...
                /*tabViews.get(position).get("normal").setBackgroundResource(R.color.c_3f3f3f);
                tabViews.get(position).get("selected").setBackgroundResource(R.color.c_323232);*/
            ...
            if (right != null) {
                ...
                /*tabViews.get(position+ 1).get("normal").setBackgroundResource(R.color.c_3f3f3f);
                tabViews.get(position+ 1).get("selected").setBackgroundResource(R.color.c_323232);*/
...
//            this.pager.removeOnPageChangeListener(pageListener);
    ...
    public interface OnPagerTitleItemClickListener {
         ...
         * @param color
         */
        void setClickBackColor(int color);
    }
    ...
    class ViewBean {
        ...
        public void setPoston(int poston) {
            this.poston = poston;
        }
